...
**Deliverable status: v0.3.91 with ChatDev worker native build deps (opt-in image installs
build-essential + cairo headers for `uv sync`/pycairo; purge toolchain after sync; keep
`libcairo2`; ADR-073), v0.3.90 with ChatDev-in-worker depth (opt-in Dockerfile `uv sync` at pin; `org.fs_corporation.chatdev_deps` label; `deps_ready` on `/chatdev/status`; gateway billed contract test; optional fail-closed smoke; no control-plane ChatDev install; no Alembic), v0.3.89 with Work-order measurement hardening (co-commit baseline/after with replay writes; `json_extract` list filter; denied-scope GET test; no Alembic; no UI/auth change), v0.3.88 with Provider invoice allocations (provider invoice headers + allocations onto immutable `billed_costs`; variance informational on summary/list/detail; desk `#budget` + companion Finance Browse/Manage provider group; Alembic 0030; no Stripe/auto-adjust/billed mutate), v0.3.87 with Consultant work-order measured before/after (baseline/after ops snapshots on authorize/complete; GET measurements with arithmetic deltas; desk `#consultant` + companion Home Needs-you; Alembic 0029; no invented scores), v0.3.86 with Finance open-next budget period (`POST …/open-next` after close; pricing honesty on finance summary; desk + companion Open next + hint; no Alembic; no auto-rollover), v0.3.85 with Desk remaining session gates (Activate/promotions/staffing organization.write; dispatch submit/recommend project.enroll; README/VERIFICATION honesty; no new APIs), v0.3.80 with Desk Finance polish (session pause gate via /api/v1/session; openRoom simulated + reserved spend; #budget-scoped dump ban; no new APIs), v0.3.79 with Companion finance URL polish (drop dead Close-period Manage focus; cold-load/popstate use mode-aware defaultGroupFor; tsx urlState behavioral harness; no new APIs), v0.3.78 with Desk Finance surface (expand `#budget` into Finance label + structured summary/lists/forms/close; section id unchanged; desk-native long scroll; no ModeSwitch or new APIs), v0.3.77 with Finance Browse/Manage (ModeSwitch + ManageClusters lists-vs-forms; finance in MODE_CAPABLE_TABS; `group` in browse+manage; close-period stays Browse in-row; no new APIs), v0.3.76 with mode/cluster/group companion URL sync (`mode`/`cluster`/`group` query params with App-owned state, replaceState, omit defaults, controlled panels; pairing hash unchanged; no new APIs), v0.3.75 with empty-list unknown-project URL clear (`projectsLoaded` gates silent unknown-`?project=` clear after successful projects refresh; failed fetch keeps deep link; no new APIs), v0.3.74 Manage visual groups on Organization, Corporate, Projects and Workers via shared `useWideViewport` + `ManageClusters` (labeled scroll ≥720px; segmented group tabs below; no URL sync or API changes), v0.3.73 companion URL sync for `?tab=` and `?project=` deep links (`replaceState`; unknown project clears silently; leaving Projects or Clear drops `project`; pairing hash unchanged), v0.3.72 Projects Browse list-row `span.muted` and Clear-on-loading (detail-toolbar while loading; empty pane without Clear), v0.3.71 Corporate Browse clusters (Strategy · Structure · People · Coordination) and narrow-viewport segmented tabs plus light Manage section-head titles, Organization Browse/Manage section-head consistency, Corporate/Workers Browse section consistency, companion Projects Browse list|detail split and medium Work/People shell polish, desk five-domain IA and companion Work/People Browse–Manage structure and Corporate HQ Phases 1–8 complete, including a public cosmic-glass `/welcome` page and marketing `campaign` furniture, a persisted-operations CEO scorecard and governed objectives, approval-gated divisions/industry packs, staffing proposals, career ladders/promotions, live activity projection, worker identity, floorplans, and runtime organization editing + prior organization roster/head handoff UI + local-repos candidates + Diagnostics + phone GitHub assign-by-address + billed cost/revenue tables + companion PWA build (generateSW) + worker-completion transaction + atomic idempotency + Alembic-on-startup + HTTP 429 + same-host worker plane on `.101` + live github.com Funnel webhooks (ping+push) + impact-brief API + live GitHub pilot + model/feed adapters + Docker/container workers + Web Push + ChatDev worker live path (control-plane deny by default) + optional ChatDev worker image labels on `/chatdev/status` + P4 worker-host registry/heartbeat + SVG HQ furniture + TailscaleKit stub + P5 shared tokens/version/HQ keyboard.** Projects Browse uses a responsive list|detail split with dispatch in the detail pane and Clear selection; Manage remains enroll/assign. Corporate Browse uses Strategy · Structure · People · Coordination clusters with cluster-head labels and segmented tabs below 720px (shared viewport hook); Manage groups on Org/Corporate/Projects/Workers; Workers title sits outside the list card; Organization Browse uses a Departments section-head and Manage form titles sit outside form cards; Finance uses Browse/Manage with Overview · Invoices · Adjustments · Periods in Browse and create forms in Manage. Billed model pricing needs `FS_CORP_MODEL_CENTS_PER_1K_TOKENS` (or profile rate); remote pull agents default to mock completion and can opt into fail-closed container execution through a host-token gateway relay. Opt-in `FS_CORP_PREFER_REMOTE_WORKERS` auto-places on the first ready remote when `worker_host_id` is omitted (fail closed). Remote claims now carry the company ChatDev egress policy; container agents use a named allowlisted Docker network only when locally ready and otherwise fail the job, while `none` remains `--network none`.
...
| ChatDev, GitHub and market integration interfaces | GitHub live when App configured; signed webhooks when `GITHUB_WEBHOOK_SECRET` set; feed poll live for approved RSS/Atom URLs; ChatDev opt-in when `CHATDEV_HOME` set (`GET /api/v1/chatdev/status` incl. `worker_image_chatdev` with optional `deps_ready`; worker live when `chatdev: true`; control-plane needs `CHATDEV_ALLOW_CONTROL_PLANE=1`); opt-in worker Docker image may include uv-synced ChatDev deps when `CHATDEV_ENABLE=1`; doc fetch still disabled |
...
| Remote worker host registry | Implemented: CEO CRUD + heartbeat; explicit `worker_host_id` pull jobs; default mock-complete, with opt-in container execution through a host-token gateway relay; claims carry ChatDev egress mode/network (never hostnames), allowlist execution fails closed unless the agent is locally ready; default dispatch same-host |
...
| Mobile CEO companion PWA + dashboard APIs | Implemented; five-domain shell (Home · Work · People · Money · More) with persisted Home Needs-you queue; org catalog/seats/roster, head inbox/assign, dormant-department activation, and explicit department budgets; LAN HTTPS and Tailscale access documented; Web Push live when VAPID keys configured; native Expo shell for iOS/Android Tailscale handoff |
