...
**Companion finance URL polish 0.3.79 is on `main` and fs-dev** (tip `72e4cea`).
Close period stays on Browse; cold-load/popstate use mode-aware group defaults;
`urlState` harness runs from unittest discover. Next: owner-directed desk polish
nits (403-only pause gate / openRoom reserved-only) or other companion
follow-ups.
