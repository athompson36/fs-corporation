...
**Desk Finance surface 0.3.78 is on branch `feature/desk-finance-surface`**
(Tasks 1–5 complete; pending merge/deploy). Desk `#budget` expands to Finance
summary/lists/forms/close; section id unchanged; no ModeSwitch or new APIs.
Companion Finance Browse/Manage 0.3.77 remains on `main` and fs-dev (tip
`5b21dcd`). Next: owner-directed polish nits (dead `closePeriod` focus;
cold-load `defaultGroupFor` bias; behavioral URL tests) or other companion
follow-ups.
