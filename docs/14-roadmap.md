...
**v0.3.86 status:** Finance open-next budget period (`POST …/open-next` after close; pricing honesty on finance summary; desk + companion Open next + hint; ADR-068; no Alembic; no auto-rollover on close). Joins **v0.3.85 status:** Desk remaining session gates fail-close division Activate, promotion decisions, staffing scan/decisions via extended `organization.write` gate; dispatch submit/recommend via `project.enroll` and `setDispatchEnrollEnabled`; README/VERIFICATION honesty; no new APIs. Joins **v0.3.84 status:** Desk corporate write forms session gate extends `organization.write` fail-closed to `#scorecard`, `#cross-department`, and `#corporate-upgrades` (three static submits + dynamic Close/Accept); per-section `data-org-write-notice`; extended `setOrgMutateEnabled` + `orgWriteEnabled`; 403 backup on Close/Accept; promotions/staffing ungated; no new APIs. Joins **v0.3.83 status:** Desk Organization session mutate gate fail-closes Organization Manage in `#departments` via shared `/api/v1/session` (`organization.write` scope); seven submit chips ship disabled with visible `#org-scope-notice`; init `setOrgMutateEnabled(false)`; Finance session/`company.pause` unchanged; 403 backup in `submitOrgCommand`; no new APIs. Joins **v0.3.82 status:** Desk Finance init-time mutate disable ships Finance mutate controls `disabled` with visible `#finance-scope-notice`; calls `setFinanceMutateEnabled(false)` at init; session/403 paths unchanged; no new APIs. Joins **v0.3.81 status:** Companion tab/mode URL flash fix batches sibling resets in `selectTab` / `handlePanelModeChange` via `stateAfterTabChange` / `stateAfterModeChange`; removes lagging reset effects; extends tsx `urlState` harness for transition sequences; no Router or new APIs. Joins **v0.3.80 status:** Desk Finance polish gates Finance mutations via `/api/v1/session` (`company.pause` scope); restores HQ openRoom simulated + reserved spend; scopes dump ban to `#budget` only; no new APIs. Joins **v0.3.79 status:** Companion finance URL polish drops dead Close-period Manage focus; cold-load/popstate use mode-aware `defaultGroupFor`; tsx `urlState` behavioral harness; no new APIs. Joins **v0.3.78 status:** Desk Finance surface expands `#budget` into Finance label + summary/lists/forms/close; section id unchanged; desk-native long scroll; no ModeSwitch or new APIs. Joins **v0.3.77 status:** Finance Browse/Manage (ModeSwitch + ManageClusters lists-vs-forms; finance in MODE_CAPABLE_TABS; `group` in browse+manage; close-period stays Browse in-row; no new APIs) joins **v0.3.76 status:** Mode/cluster/group companion URL sync (`mode`/`cluster`/`group` query params with App-owned state, replaceState, omit defaults, controlled panels; pairing hash unchanged) joins v0.3.75 empty-list unknown-project URL clear (`projectsLoaded` gates silent unknown-`?project=` clear after successful projects refresh), v0.3.74 Manage visual groups on Organization, Corporate, Projects and Workers via shared `useWideViewport` + `ManageClusters`, v0.3.73 Companion URL sync for `?tab=` and `?project=` with replaceState, v0.3.72 Projects list-row `span.muted` and Clear-on-loading, v0.3.71 Corporate Browse clusters with narrow-viewport segmented tabs and light Manage section-head titles, the prior v0.3.70 Org Browse/Manage polish, v0.3.69 Corporate/Workers section-head consistency, v0.3.68 Projects Browse split, cosmic-glass desk and companion, isometric HQ, SLO catalog, Web Push, feed poll, GitHub effect, container file gateway, same-host worker plane on `.101`, and fs-dev deployment. Live adapters are **opt-in and fail-closed**: they raise `NotImplementedError` until the owner supplies credentials, and on the fs-dev host GitHub webhooks, the market feed, and model invocation have run live. Live ChatDev execution and billed model calls from inside worker containers remain unavailable. The deployed fs-dev health is **0.3.80** (Desk Finance polish installed).
...
- [x] Empty-list unknown-project URL clear (0.3.75) — `projectsLoaded` gates
      silent unknown-`?project=` clear after successful refresh; failed fetch
      keeps deep link; no new APIs, Manage/Browse URL sync or Finance ModeSwitch
- [x] Mode/cluster/group URL sync (0.3.76) — `mode`/`cluster`/`group` deep
      links with App-owned state and replaceState; omit browse/strategy/default
      group; controlled panels; Workers token → `group=token`; pairing hash
      unchanged; no Router, Finance ModeSwitch or new APIs
- [x] Finance Browse/Manage (0.3.77) — Finance ModeSwitch + ManageClusters
      lists-vs-forms; finance in MODE_CAPABLE_TABS; `group` in browse+manage;
      close-period stays Browse in-row; no new APIs or Alembic revision
- [x] Desk Finance surface (0.3.78, companion-adjacent) — expand desk `#budget`
      into Finance label + summary/lists/forms/close; keep section id; desk-native
      long scroll; no ModeSwitch or new APIs
- [x] Companion finance URL polish (0.3.79) — drop dead Close-period Manage focus;
      cold-load/popstate use `defaultGroupFor(tab, mode)`; tsx `urlState` harness;
      no new APIs
- [x] Desk Finance polish (0.3.80, companion-adjacent) — session pause gate via
      `/api/v1/session`; openRoom simulated + reserved spend; `#budget`-scoped dump
      ban; no new APIs
- [x] Companion tab/mode URL flash (0.3.81) — batch tab/mode sibling resets in
      `selectTab` / `handlePanelModeChange`; remove lagging reset effects; extend tsx
      harness; no Router or new APIs
- [x] Desk Finance init-time mutate disable (0.3.82, companion-adjacent) — ship
      mutate controls disabled with visible `#finance-scope-notice`; call
      `setFinanceMutateEnabled(false)` at init; session/403 paths unchanged; no new APIs
- [x] Desk Organization session mutate gate (0.3.83, companion-adjacent) — ship
      Organization Manage fail-closed via session `organization.write`; seven
      `#departments` submit chips disabled at init with visible notice; shared session
      with Finance; 403 backup; no new APIs
- [x] Desk corporate write forms session gate (0.3.84, companion-adjacent) — ship
      Corporate write forms fail-closed via extended `organization.write` gate; three
      static submits + dynamic Close/Accept disabled at init with per-section notices;
      extended `setOrgMutateEnabled`; 403 backup; no new APIs
- [x] Desk remaining session gates (0.3.85, companion-adjacent) — ship
      People/Activate/promotions/staffing via `organization.write`; dispatch
      submit/recommend via `project.enroll` and `setDispatchEnrollEnabled`; README/
      VERIFICATION honesty; no new APIs
- [x] Finance open-next and pricing honesty (0.3.86) — ship explicit
      `POST …/open-next` after close; pricing honesty on finance summary; desk +
      companion Open next + hint; ADR-068; no Alembic; no auto-rollover
...
**Finance open-next and pricing honesty 0.3.86** ships on branch
`feature/finance-open-next-pricing`. Explicit open-next after close, pricing honesty on
finance summary, desk + companion UI; version lockstep at 0.3.86.
Next: Ship 2 remaining — consultant measured before/after (0.3.87; brainstorm/plan
separately).
