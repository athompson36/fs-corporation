...
**Desk Finance polish 0.3.80 is on `main` and fs-dev** (tip `1085ac9`).
Session-scoped Finance mutation gate; openRoom spend restored; companion
version lockstep at 0.3.80. Next: owner-directed (companion one-frame
tab-change URL flash; init-time Finance disable before session; or other
follow-ups).
