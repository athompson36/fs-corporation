...
`approve_feed_source` is CEO-only and records an HTTPS URL. `poll_market_feed` requires that enrollment, writes an idempotent `feed_polls` row, then calls `MarketFeedAdapter.poll`. Until a live adapter is configured the row becomes `live_unavailable`; no signal is invented and policy does not change.
