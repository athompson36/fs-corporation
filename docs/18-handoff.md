...
- `assign_dispatch` requires an assignable dispatch, a live head seat plus
  `work.assign`/project and an explicit nonempty matching department grant (unless
  CEO/admin companion), and a rostered specialist or project-granted contractor.
  Successful assignment creates a real queue row.
...
- Vacating a head blocks that head's remaining open dispatches and cancels queued or
  leased queue rows linked through `dispatch_assignments`.
...
```bash
.venv/bin/python -m unittest tests.test_org_handoff tests.test_org_roster -v
.venv/bin/python -m unittest discover -s tests
```

Latest review-fix verification: 24 focused tests and 246 full-suite tests passed.
