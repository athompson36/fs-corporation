...
The loopback service authenticates consultant principals with `consultant.read` / `consultant.propose` and requires `consultant.decide` for CEO decisions. `revise` creates a new digest. `to_work_order` records an authorized work order without dispatching. `GET /api/v1/consultant/reviews` exposes persisted review cooldown rows for operator honesty. Measured before/after efficiency validation still requires a live change and is not invented by the API. Production container isolation for ChatDev (allowlisted egress when configured), live AI analysis and scheduled triggers remain separate ops work.
