...
Updated 2026-09-07 for **0.3.48** (billed cost / revenue). Prior: 0.3.47 companion PWA;
0.3.46 M10-02; 0.3.45–0.3.42 M10-01; 0.3.41 same-host worker plane. Run on Python 3.14.3
in `.venv` on macOS / Node 18.20.8. CI additionally runs Python 3.12 and 3.13
...
- **207 unit tests pass** via `python -m unittest discover -s tests`, including
  `tests.test_m10_finance` (billed persistence, revenue, unmixed totals) and updated
  `tests.test_model_feed_live` (usage_tokens + optional pricing).
...
- Alembic revision chain is linear and single-headed: `0001_initial` through
  `0013_billed_cost_revenue`.
- Companion build completes (`generateSW`) and emits `dist/sw.js` / `dist/sw-push.js`
  (companion **0.3.7**).
