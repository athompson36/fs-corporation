...
Updated 2026-09-07 for **0.3.43** (Alembic on startup). Prior: 0.3.42 HTTP 429; 0.3.41
...
- **194 unit tests pass** via `python -m unittest discover -s tests`, including
  `tests.test_rate_limit` and `tests.test_migrate` (legacy file DB gains `access_level` via
  Alembic). Run twice: clean environment and with developer `.env` exported.
...
- HTTP `429` rate limiting and Alembic-on-startup are implemented (0.3.42–0.3.43). Remaining
  M10 gaps: atomic idempotency, adapter `cancel`/`fail` mapping tests, consultant
